...
static char szSearchPath[MAX_PATH];
...
BOOL wbSysDlgOpen(PWBOBJ pwboParent, LPCTSTR pszTitle, LPCTSTR pszFilter, LPCTSTR pszPath, LPTSTR pszFileName)
{
	...
	if(pszPath && *pszPath)
		pszCopy = MakeWinPath(wbStrDup(pszPath));
	else
		pszCopy = NULL;
	...
	ofn.hInstance = NULL;
	ofn.lpstrFilter = pszFilter && *pszFilter ? pszFilter : "All files (*.*)\0*.*\0\0";
...
static LPTSTR StripPath(LPTSTR pszFileName)
{
	...
	if((szPos = strchr(pszFileName, '\\')) != NULL)
