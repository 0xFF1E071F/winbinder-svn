...
HMODULE wbLoadLibrary(LPCTSTR pszLibName)
{
	...
	if(!hLib) {
		wsprintf(szName, "%s32", pszLibName);
		hLib = LoadLibrary(szName);
		if(!hLib) {
			wsprintf(szName, "%s.exe", pszLibName);
			hLib = LoadLibrary(szName);
			if(!hLib) {
				wsprintf(szName, "%s32.exe", pszLibName);
				hLib = LoadLibrary(szName);
			}
		}
	}
...
FARPROC wbGetLibraryFunction(HMODULE hLib, LPCTSTR pszFunction)
{
	...
	for(i = 0; i <= 20; i++) {
		wsprintf(szName, "%s@%d", pszFunction, i * 4);
		pFn = GetProcAddress(hLib, szName);
		if(pFn)
			return pFn;
	}
	...
	for(i = 0; i <= 20; i++) {
		wsprintf(szName, "_%s@%d", pszFunction, i * 4);
		pFn = GetProcAddress(hLib, szName);
		if(pFn)
			return pFn;
	}
