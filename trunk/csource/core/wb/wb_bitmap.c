...
HANDLE wbLoadImage(LPCTSTR pszImageFile, UINT nIndex, LPARAM lParam)
{
	...
	strncpy(szFile, pszImageFile, MAX_PATH - 1);

	if(!wbFindFile(szFile, MAX_PATH)) {
		return NULL;
	}

	if(strstr(szFile, ".bmp")) {								// Load bitmap
		...
		if(!hbmpData) {
			wbError(__FUNCTION__, MB_ICONWARNING, "Invalid bitmap format in file %s", szFile);
			return NULL;
		}
	...
	} else if(strstr(szFile, ".ico") || strstr(szFile, ".icl") ||
	  strstr(szFile, ".dll") || strstr(szFile, ".exe")) {	 // Load icon
		...
		if(lParam == 0) {
			...
			hIcon = hIconBuf[0];
		} else if(lParam == -1) {								// Returns the default icon (not used anymore)
		...
		if(hIcon == (HICON)1) {
			wbError(__FUNCTION__, MB_ICONWARNING, "File %s does not contain any icons.", szFile);
			return NULL;
		} else if(hIcon == NULL) {
			if(nIndex == 0)
				wbError(__FUNCTION__, MB_ICONWARNING, "No icons found in file %s", szFile);
			else
				wbError(__FUNCTION__, MB_ICONWARNING, "Icon index #%d not found in file %s", nIndex, szFile);
			return NULL;
		} else {
			return hIcon;
		}
...
static HBMP ReadBitmap(LPCTSTR szBMPFileName)
{
	...
	if(-1 == (hFile = _lopen (szBMPFileName, OF_READ | OF_SHARE_DENY_WRITE)))
		return NULL;
