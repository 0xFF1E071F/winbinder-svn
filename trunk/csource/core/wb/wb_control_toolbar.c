...
PWBOBJ wbCreateToolbar(PWBOBJ pwboParent, PWBITEM pitem[], int nItems, int nBtnWidth, int nBtnHeight, HBITMAP hbm)
{
	...
	if(!hToolbar) {
		wbError(__FUNCTION__, MB_ICONWARNING, "Could not create toolbar");
		return NULL;
	}
...
static BOOL CreateToolbarButton(HWND hwnd, int id, int nIndex, LPCTSTR pszHint)
{
	...
	if(id == 0) {			// Separator
		...
		if(pszHint && *pszHint) {
			tbb.dwData = (DWORD)wbStrDup(pszHint);
		} else
			tbb.dwData = 0;
		tbb.iBitmap = nIndex;
		tbb.iString = nIndex;
	...
	if(!bRet) {
		if(id)
			wbError(__FUNCTION__, MB_ICONWARNING, "Could not create item # %d in toolbar", id);
		else
			wbError(__FUNCTION__, MB_ICONWARNING, "Could not create separator in toolbar");
	}
	return bRet;
}
