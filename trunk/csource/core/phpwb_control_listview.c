...
ZEND_FUNCTION(wbtemp_create_listview_item)
{
	...
	newitem = wbCreateListViewItem((PWBOBJ)pwbo, nitem, nimage, s);
...
ZEND_FUNCTION(wbtemp_set_listview_item_text)
{
	...
	RETURN_BOOL(wbSetListViewItemText((PWBOBJ)pwbo, item, sub, s));
}
...
ZEND_FUNCTION(wbtemp_create_listview_column)
{
    ...
    if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,
	  "llsl", &pwbo, &oo, &s, &s_len, &w) == FAILURE)
		return;

	if(!wbIsWBObj((void *)pwbo, TRUE))
		RETURN_NULL()

	RETURN_BOOL(wbCreateListViewColumn((PWBOBJ)pwbo, oo, s, w));
}
	...
	if(nitem >= 0) {
		...
		if(ncol >= 0) {										// A single cell
			...
			for(ncol = 0; ncol < ncols; ncol++) {
				if(wbGetListViewItemText((PWBOBJ)pwbo, nitem, ncol, szItem, MAX_ITEM_STRING - 1))
					add_next_index_stringl(return_value, szItem, strlen(szItem), 1);
				else
