...
{
    long wbclass, x, y, w, h, style, lparam;
	char *caption = "";
	int caption_len = 0;
    ...
    if(zend_parse_parameters(nargs TSRMLS_CC,
	 "ll|sllllll", &pwboparent, &wbclass, &caption, &caption_len, &x, &y, &w, &h, &style, &lparam) == FAILURE)
		return;
	...
	pwbo = (long)wbCreateWindow((PWBOBJ)pwboparent, wbclass, caption, x, y, w, h, 0, style, lparam);
...
ZEND_FUNCTION(wb_get_instance)
{
	...
	RETURN_BOOL(wbGetRequestedAppWindow(caption, bringtofront) != NULL);
}
