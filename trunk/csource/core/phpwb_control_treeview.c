...
ZEND_FUNCTION(wbtemp_set_treeview_item_text)
{
	...
	RETURN_BOOL(wbSetTreeViewItemText((PWBOBJ)pwbo, (HTREEITEM)item, s));
}

ZEND_FUNCTION(wbtemp_get_treeview_item_text)
{
	LONG pwbo, item = 0;
	char szItem[MAX_ITEM_STRING];
...
ZEND_FUNCTION(wbtemp_create_treeview_item)
{
	...
	switch(insertiontype) {
		...
		default:
			RETURN_LONG((LONG)wbAddTreeViewItemLevel((PWBOBJ)pwbo, where, str, lparam, setlparam, img1, img2));
			break;
		case 1:						// 'where' is the sibling of the new node
			RETURN_LONG((LONG)wbAddTreeViewItemSibling((PWBOBJ)pwbo, (HTREEITEM)where, str, lparam, setlparam, img1, img2));
			break;
		case 2:						// 'where' is the parent of the new node
			RETURN_LONG((LONG)wbAddTreeViewItemChild((PWBOBJ)pwbo, (HTREEITEM)where, str, lparam, setlparam, img1, img2))
			break;
	}
}
