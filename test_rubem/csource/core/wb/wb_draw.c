...
BOOL wbGetTextSize(PSIZE psizeText, LPCTSTR pszString, int nFont)
{
	...
	bRet = GetTextExtentPoint32(hdcTest, (LPTSTR)pszString, strlen(pszString), psizeText);
...
BOOL wbDrawText(HANDLE handle, LPCTSTR pszString, int xPos, int yPos, int nWidth, int nHeight, int nFont, DWORD dwFlags)
{
	...
	if(nWidth > 0 && nHeight > 0) {
		...
		bRet = DrawTextEx(hdcMain, (PSZ)pszString, nLen, &rc, dwWinFlags, NULL);
	} else {
		bRet = TextOut(hdcMain, xPos, yPos, (PSZ)pszString, nLen);
	}
